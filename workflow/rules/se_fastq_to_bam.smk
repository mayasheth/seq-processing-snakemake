rule align_se_fastq:
  ...
  """
  input:
      fq = lambda wildcards: get_fastq_files(wildcards.accession, SAMPLE_CONFIG, config, SCRATCH_DIR)[0],
  ...
  output:
      bam = temp(os.path.join(SCRATCH_DIR, "{sample_name}", "{assay}", "{accession}.se.bam"))
  log:
...
rule post_filter_se_bam:
  ...
  output:
      filtered = temp(os.path.join(SCRATCH_DIR, "{sample_name}", "{assay}", "{accession}.se.filtered.sorted.bam"))
  params:
      scratch = SCRATCH_DIR,
      mapq    = MAPQ
  log:
...
rule markdup_se_bam:
  ...
  output:
      dedup = os.path.join(RESULTS_DIR, "{sample_name}", "{assay}", "{accession}.se.filtered.sorted.dedup.bam"),
      bai = os.path.join(RESULTS_DIR, "{sample_name}", "{assay}", "{accession}.se.filtered.sorted.dedup.bam.bai")
  params:
      scratch = SCRATCH_DIR
  log:
